...
public class PotionEvents {
    ...
    public static void onBlockBreak(BlockEvent.BreakEvent event) {
        ...
        if (!player.isCreative() && player.hasEffect(ModEffects.GATHERERS_GLEE_EFFECT.get())) {
            ...
            if (ModBlocks.VALID_BLOCKS.contains(event.getState().getBlock())) {
                ...
                ItemEntity item = new ItemEntity((Level) event.getWorld(), event.getPos().getX(), event.getPos().getY(), event.getPos().getZ(), new ItemStack(event.getState().getBlock(), 1));
    ...
    public static void onBlockBreak2(BlockEvent.BreakEvent event) {
        ...
        ItemEntity dirt = new ItemEntity((Level) event.getWorld(), event.getPos().getX(), event.getPos().getY(), event.getPos().getZ(),
                new ItemStack(Item.byBlock(Blocks.DIRT), 1));
        ItemEntity item = new ItemEntity((Level) event.getWorld(), event.getPos().getX(), event.getPos().getY(), event.getPos().getZ(),
        ...
        if (ModBlocks.GRASS.contains(event.getState().getBlock())) {
            ...
            if (!player.isCreative() && player.hasEffect(ModEffects.FLORAL_FORTUNE_EFFECT.get()) ||
                ...
                event.getWorld().addFreshEntity(dirt);
